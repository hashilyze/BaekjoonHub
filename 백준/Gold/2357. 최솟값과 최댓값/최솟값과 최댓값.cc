...
std::pair<int, int> initSegment(int nodeL, int nodeR, int index){ 
    if(nodeL == nodeR) return { segmentMin[index] = arr[nodeL], segmentMax[index] = arr[nodeL] };
    
    int mid = (nodeL + nodeR) >> 1;
    auto lhs = initSegment(nodeL, mid, index * 2);
    auto rhs = initSegment(mid + 1, nodeR, index * 2 + 1);
    return {
